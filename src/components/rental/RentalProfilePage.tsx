...
export default function RentalProfilePage({profile,rental_id}: { profile: RentalProfile ; rental_id: string}) {
  ...
  const handleRentFriend = async() => {
    ...
    // Simulate booking process
      setIsBooking(false);
      toast(
         `Request sent! Hope ${profile.name} is free to chill soon 💬`
      );
    }
    ...
    if(request){
      ...
      if(NotifyError){
        console.error('Error in sending booking notification:', NotifyError.message)
      }
    }
    setOpen(false)
    setLoading(false);
  ...
  useEffect(()=>{
    const fetchReview= async()=>{
      try {
        ...
        .from('reviews')
        .select( `id,rentalprofile_id,user_id,rating,comment,created_at,profiles:user_id(name,avatar)`)
  ...
  if (isOwner) {
    return (
      ...
      <div className="max-w-4xl mx-auto">
        <h1 className="px-auto py-1 text-lg text-red-500 ">NOTE : Accepting payment is on you, after service or before service.</h1>
            ...
            <Card>
              <CardContent className="space-y-4">
                ...
                <div className="mt-6">
                  ...
                  <p className="text-gray-700 break-words whitespace-pre-line">{profile.bio}</p>
                </div>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
            <DialogHeader>
              <DialogTitle className="text-xl tracking-tight border-b-2 pb-2 font-sans">Send a slot booking request</DialogTitle>
            </DialogHeader>
            <div className="grid gap-6 py-4">
          <div className=''>
            <Label htmlFor="service" className="text-lg font-bold text-right">
              Services you want...
            </Label>
            <Input id="service" onChange={(e)=>setService(e.target.value)} className="col-span-3" placeholder="Career advice, Dating, etc..."/>
          </div>
          <div className=" items-center gap-4">
            <Label htmlFor="time" className="text-lg font-bold text-right">
              When
            </Label>
            <Input id="time" onChange={(e)=>setDateTime(e.target.value)} type="datetime-local" className="col-span-3" />
          </div>
          <div className="items-center gap-4">
            <Label htmlFor="duration" className="text-lg font-bold text-right">
              Duration
              <span className="text-[10px] line-clamp-1 tracking-tighter ">In Hours</span>
            </Label>
            <Input id="duration" onChange={(e)=>setDuration(e.target.value)} type="number" className="col-span-3" />
          </div>
          <div className="items-center gap-4">
            <Label htmlFor="message" className="text-lg font-bold text-right">
             Message
            </Label>
            <Input id="message" onChange={(e)=>setMessage(e.target.value)} placeholder="Any instruction" type="text" className="col-span-3" />
          </div>
        </div>
        <DialogFooter>
          <Button onClick={handleRentFriend}> Send Request</Button>
        </DialogFooter>
              </DialogContent>
          </Dialog>
              </CardFooter>
